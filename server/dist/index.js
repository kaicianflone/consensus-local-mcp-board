...
app.get('/api/mcp/boards/:id', (req, res) => {
    ...
    if (!board)
        return res.status(404).json(err('BOARD_NOT_FOUND', 'Board not found'));
    res.json({ board, runs: listRuns(req.params.id, 100) });
});
app.get('/api/mcp/runs/:id', (req, res) => {
    const run = getRun(req.params.id);
    if (!run)
        return res.status(404).json(err('RUN_NOT_FOUND', 'Run not found'));
...
app.post('/api/mcp/tool/:name', (req, res) => {
    try {
        const out = invokeTool(req.params.name, req.body ?? {});
        res.json(out);
    }
    catch (e) {
