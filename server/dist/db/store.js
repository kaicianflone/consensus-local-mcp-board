...
runMigrations(db);
export function createBoard(name, config = {}) {
    const id = nanoid();
    const ts = Date.now();
...
export function getBoard(id) { return db.prepare('SELECT * FROM boards WHERE id = ?').get(id); }
export function createRun(boardId, meta = {}) {
    const id = nanoid();
    const ts = Date.now();
...
export function listRuns(boardId, limit = 100) {
    if (boardId)
        return db.prepare('SELECT * FROM runs WHERE board_id=? ORDER BY created_at DESC LIMIT ?').all(boardId, limit);
...
export function appendEvent(boardId, runId, type, payload) {
    const id = nanoid();
    const prev = runId ? db.prepare('SELECT id FROM events WHERE run_id=? ORDER BY ts DESC LIMIT 1').get(runId) : undefined;
